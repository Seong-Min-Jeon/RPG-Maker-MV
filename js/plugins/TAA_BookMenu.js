 ...
 * @target MV MZ
 *
 * @plugindesc [1.5.2] Create a Book Menu
...
LibraryData.prototype.imgExists = function(file){
    var fs = require('fs');
    var filePath = this.inlineImageFolder() + file + ".png";

    if(this._isDeployedProject === undefined){
        if(fs.existsSync(filePath)){
            this._isDeployedProject = false;
            return true;
        }
        else if(fs.existsSync("www/" + filePath)){
            this._isDeployedProject = true;
            return true;
        }
    }
    if(this._isDeployedProject !== undefined){
        if(this._isDeployedProject)
...
Scene_BookMenu.prototype.getWindowLayerIndex = function(){
    ...
    while(index >= 0 && !found){
        ...
        var textIndex = obj.children.length - 2;
